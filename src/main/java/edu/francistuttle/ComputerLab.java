                ...
                if (node.getNodeType() == Node.ELEMENT_NODE)
                {
                    ...
                    for (int j = 0; j < nList2.getLength(); j++)
                    {
                        ...
                        if (childNode.getNodeType() == Node.ELEMENT_NODE)
                        {
                            ...
                            mList.add(new Monitor(eElement2.getAttribute("id"), eElement2.getElementsByTagName("manufactor").item(0).getTextContent(), eElement2.getElementsByTagName("resolution").item(0).getTextContent()));
                        }
                    }
                    computers.add(new Computer(eElement.getAttribute("id"), eElement.getElementsByTagName("manufactor").item(0).getTextContent(), eElement.getElementsByTagName("yearPurchased").item(0).getTextContent(), eElement.getElementsByTagName("processor").item(0).getTextContent(), mList));
                }
            }
            return computers;
            } 
